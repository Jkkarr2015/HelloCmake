...
bool OSMPOISearch::isPOISimilarToPOI(POI a, POI b) {
    ...
    if (a.isEqualToPOI(b))
        bRes = true;
    else if (!a.getName().empty() && !b.getName().empty() && a.getName().compare(b.getName()) == 0) {
        if (a.getAddress().empty() && b.getAddress().empty())
            bRes = true; //no address and names match, similar
        else if (!a.getAddress().empty() && !b.getAddress().empty() && a.getAddress().compare(b.getAddress()) == 0)
...
vector<POI> OSMPOISearch::searchPOIpreproc (Location loc ,string searchString ,double radius, int limit ,double level ,bool sortResults) {
    ...
    double aclldlat = loc.getLatitude();
    double aclldlon = loc.getLongitude();
    ...
    aopr.open("/storage/emulated/0/Android/data/com.example.jkarr.hello_cmake/files/kentucky-latest_w2n_POI.osm.pbf", aclldlat, aclldlon, radius, &vOSMnode);
    ...
    for(int i=0;i<vOSMnode.size();i++){
        ...
        if (level && (!poi.getFloor() || !(level == poi.getFloor()))) {
        ...
        if(vOSMnode.at(i).HasTag("website")){
            poi.setWebsite(vOSMnode.at(i).GetTag("website").c_str());
        }

        if(vOSMnode.at(i).HasTag("phone")){
            poi.setPhone(vOSMnode.at(i).GetTag("phone").c_str());
        }
        ...
        poi.getLocation().setLatitude(dLat);
        poi.getLocation().setLongitude(dLon);
        poi.setBearing(loc.bearingTo(poi.getLocation()));
        poi.setDistance(loc.distanceTo(poi.getLocation()));

        poi.setId((std::to_string(vOSMnode.at(i).id)));
        if(poi.getId().size() == 0)
            poi.makeId();
...
void OSMPOISearch::beaconSearch(Location *loc, double radius, int limit, bool sortResults)
{
    ...
    __android_log_print(ANDROID_LOG_DEBUG, "JDK", "Size after Open: %i", vOSMnode.size());
    for(int i = 0; i < vOSMnode.size();i++)
        vOSMnode.at(i).fAux = GetDistanceMilesHaversineApprox(vOSMnode.at(i).lat, vOSMnode.at(i).lon, ig64lat, ig64lon);
    ...
    for(int i=0;i<vOSMnode.size();i++){
        ...
        aBeacon.setLevel(atoi(vOSMnode.at(i).GetTag("level").c_str()));
        aBeacon.setMajor(atoi(vOSMnode.at(i).GetTag("beacon:major").c_str()));
        aBeacon.setMinor(atoi(vOSMnode.at(i).GetTag("beacon:minor").c_str()));
        aBeacon.setName(vOSMnode.at(i).GetTag("name").c_str());
        aBeacon.setType(vOSMnode.at(i).GetTag("beacon:type").c_str());
        aBeacon.setUUID(vOSMnode.at(i).GetTag("beacon:uuid").c_str());
      ...
      //  __android_log_print(ANDROID_LOG_DEBUG, "JDK", "Beacon-Minor: %s ",vOSMnode.at(i).GetTag("beacon:minor").c_str() );
       ...
       // __android_log_print(ANDROID_LOG_DEBUG, "JDK", "UUID: %s",vOSMnode.at(i).GetTag("beacon:uuid").c_str() );
        beaconResults.push_back(Beacon(aBeacon));
